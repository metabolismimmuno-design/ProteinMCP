...
# === Layer 3 validation ===
L3_MIN_MODELS_PASS: 3                         # 3-model 模式需 3/3 通过（AF2m 已移除，验证栈仅 2-3 个 AF3 家族模型；2-model 默认 = 2/2，见 Step 3.5）
L3_MIN_MODELS_PASS_IGGM: 2                   # IgGM path: relaxed（3-model 模式 2/3；out-of-distribution conformation tolerance）
...
# === Layer 3 pose convergence (5-seed baseline, non-optional; see Step 3.5) ===
L3_POSE_CONVERGENCE_RMSD: 4.0                 # L3.A CDR3 Cα RMSD threshold for convergence (Å, aligned on antigen); L3.B 用 iRMSD+Jaccard（见 scripts/config.py 4b 段）
...
L3B_SEEDS: 5                               # Seeds per candidate in L3.B (5-seed baseline; see Step 3.5)
...
- **工具状态**：`merge_path_pools.py` / `pool_to_fasta.py`（`--source-path` + `--cdr3-only`）/ `apply_cluster_repr.py`（`--pool/--clusters/--source-path/--cross-path-keep-all`）已实现（2026-05-16 reconcile）。`--cdr3-only` 用 `cdr_boundaries.get_cdr_boundaries()` motif 法从 `sequence` 列现算 CDR3（dedup 阶段早于 ANARCI）。

**Phase 3 — generation_freq 内生信号标注（§3.1）：**

```bash
python ~/protein-design-utils/vhh/generation_freq.py \
  --candidates {RESULTS_DIR}/val/filter/dedup_pool.csv \
  --intra-tsv {RESULTS_DIR}/val/filter/dedup/boltzgen_intra_cluster.tsv \
              {RESULTS_DIR}/val/filter/dedup/germinal_intra_cluster.tsv \
              {RESULTS_DIR}/val/filter/dedup/iggm_intra_cluster.tsv \
              {RESULTS_DIR}/val/filter/dedup/rfantibody_intra_cluster.tsv \
  --cross-tsv {RESULTS_DIR}/val/filter/dedup/cross_cluster.tsv \
  --out {RESULTS_DIR}/val/filter/dedup_pool.csv
```

- 追加两个 raw 列到 `dedup_pool.csv`（原地覆盖）：
  - `gen_cluster_size` — 候选所属 intra-path 簇成员数（生成器多次产出 = 强正信号）
  - `cross_path_count` — 候选所在 cross-path 簇内 distinct path 数（≥2 = 不同生成器独立收敛）
- 两列须随候选表一路透传到 `l4_scored.csv`（下游步骤透传未知列，与 filter_dsasa 同）。
- 派生 `generation_freq` 进 Pareto 见 Step 4.4。
...
| 非配对 Cys | CDR 区 Cys count 奇偶判断 | 奇数 → 非配对双硫桥 | **path 特化**（2026-05-16 §2）：`pathC_iggm` → **Flag**（`cdr_cys_unpaired_flag` + `rescue_suggestion="C→S/A at pos X"`，进 Pareto `seq_risk_count`；IgGM 训练无 Cys 约束→杂散 CDR Cys 点突变可救，hard drop 丢 head-on 候选）；其他 path → **Drop**（结构失败，不可救） |
...
> **Multi-seed pose convergence（5-seed 基线，非可选）：** L3.A 与 L3.B 均默认每候选 5 seeds（`L3A_SEEDS` / `L3B_SEEDS`，算力受限可 fallback 3）。L3.B 的 5-seed iRMSD+Jaccard 聚类、主簇 ipSAE_min、收敛性进 `binding_composite` 见 Step 3.5 / Step 4.4。**单 seed ipTM 0.75 不如 5 seed 中 3 个 ipTM 0.55 但姿势一致。**
...
### L3.A — Coarse co-folding screen（路径感知模型，5 seeds）
...
- Output: `{RESULTS_DIR}/val/l3b/chai1/` with per-candidate ipTM, pLDDT, PAE
...
- Output: `{RESULTS_DIR}/val/l3b/boltz2/`（仅含 Path B / C / D 候选）
...
- Output: `{RESULTS_DIR}/val/l3b/protenix/`
...
**Step 3.5 rescue — 自适应 seed budget rescue（§3.5.3）：**

5-seed pose 聚类分散（主簇 < `MAIN_CLUSTER_MIN_SIZE_BOLTZ2`）时，区分「真分散」
vs「采样不足」。`consensus_rank.py` 在输出 `consensus_ranked.csv` 时给每候选标
`needs_rescue` 列（bool）+ `rescue_models` 列。

触发判据（两条件，任一**非 Chai-1** 模型满足即 `needs_rescue=True`）：
- 该模型主簇大小 < `MAIN_CLUSTER_MIN_SIZE_BOLTZ2`（未收敛），且
- 该模型最大簇内 `ipsae_min_within` > `config.RESCUE_TRIGGER_IPSAE_MIN`（0.5）
  —— 有信号迹象，可能是采样不足而非真分散。
- **Chai-1 不参与 rescue 判定**：它用 max-epitope-hit 单代表，跨 seed 多样性近零，
  补 seed 无效；rescue 针对的是 Boltz-2 的 5-seed pose 多样性。

rescue 编排（人工触发的补算循环）：
1. 首轮 `consensus_rank.py` 跑完，筛 `needs_rescue == True` 的候选。
2. 对这些候选，按其 `rescue_models` 列所列模型补跑预测（即 Boltz-2 类多 seed 模型，
   不含 Chai-1），seed 数从 5 补到 `config.SEED_BUDGET_RESCUE_CAP`（35），新 seed
   的 CIF + 分数并入原 `l3b_root/<model>/<candidate_id>/` 目录与 `scores.json`。
3. 重跑 `consensus_rank.py`——`_load_seed_metadata` 自动读全部 seed，用扩展 seed
   集重聚类（`cluster_poses.py` 无需改）。
4. 重跑后主簇 ≥ `MAIN_CLUSTER_MIN_SIZE_BOLTZ2` → 正常进入排序；仍 < 阈值 →
   现有 `pose_diverged` flag 自然保留（binding_composite ×0.5 降权，不新设类别）。

算力账：典型 ~100 候选估 5–15 个触发（ipSAE>0.5 + 未收敛已是紧筛子）；
5→35 seeds 是 7×，仅对触发候选，总额外算力可接受。
...
python ~/protein-design-utils/vhh/l3b_hotspot_aggregate.py \
  ...
  --threshold 4.5 --min-contacts {L2P5_HOTSPOT_MIN_CONTACTS_TRACK_A} \
...
### Step 3.5a-bis — Paratope geometry & CDR-balance filter（零 GPU，4 判据 soft flag）

**Tool:** `~/protein-design-utils/vhh/paratope_geometry.py`

**Rationale:** Step 3.5a 验证「贴在正确表位」，但不验证 paratope 几何姿态。本步在 L3.B
共折叠结构上补 `dual_filter_v2.py` 的 4 个几何判据 —— skill 此前完全缺失：
- **C0** paratope 朝向（稳定 FR barrel 轴 vs hotspot patch 法向）
- **C2** H2 接触 epitope 残基数封顶（防 VHH 侧躺、H2 整段贴 patch）
- **C5** ΔH1H3 anti-flip（防 VHH 绕长 H3 翻转——实测捕到过 v3.2 通过的真假阳）
- **LAD** VHH 链 FR-FR backbone β-sheet H-bond 数（template-free framework 完整性）

**全部 soft flag**：任一判据 fail → `paratope_geometry_flag=True` → Step 4.4
`binding_composite` 乘性降权（×0.70），不 hard drop。阈值在 RFantibody 单模型 refolded
上标定，co-fold CIF 上待真 L3.B run 重校后再考虑提升 hard drop。

```bash
PYTHONPATH=~/.claude/skills/vhh-max-success-design/scripts \
python ~/protein-design-utils/vhh/paratope_geometry.py \
  --input {RESULTS_DIR}/val/l3b/hotspot_recheck.csv \
  --out {RESULTS_DIR}/val/l3b/paratope_geometry.csv \
  --epitope {EPITOPE_AUTH_RESIDUES} --hotspots {HOTSPOT_AUTH_RESIDUES} \
  --offset {ANTIGEN_SEQ_OFFSET} \
  --cif-path-col representative_cif_path_primary
```

- Input: `hotspot_recheck.csv`（Step 3.5a 输出；读 `representative_cif_path_primary` 列）
- 零额外 GPU：复用已完成的 L3.B 共折叠结构
- CDR 边界默认走 ANARCI（从 CIF VHH 链重建序列）；可 `--cdr1/--cdr2/--cdr3` 显式覆盖
- `--path-mode {default|iggm|widepath}`：C5 阈值按 `config.PATH_MODE_BY_PATH` 取
  （`iggm`=1.0Å 收紧，其余=8.0Å）。**IgGM 路径特化（§2，2026-05-16 定稿）：仅差异化
  C5**——16F9 实测 11 标注候选 ΔH1H3 在 keep（−4.5～+1.0）与 side-on（+1.6～+3.7）间有
  0.6Å 干净 gap，C5≤1.0Å 单判据已分离 IgGM 系统性 H1-lifted 侧躺；不加 C2 放宽/C6
  proximity（与 C5 信号冗余）。C0 始终用 θ(稳定FR主轴 vs patch法向)+cos_aim fallback，
  **不单用 NC·N**（长 H3 真 binder 倾斜下仍 reach patch，NC·N 单门控会误杀）
- 几何阈值单一 source-of-truth 在 `config.py` (4c) 段 —— 命令前置 `PYTHONPATH` 指向
  skill `scripts/` 目录，使 `paratope_geometry.py` 能 `import config`；省略则回退到模块
  内置默认值（数值与 (4c) 一致，但后续调 (4c) 不会生效）
- 新增列：`c0_ok` `c2_ok` `c5_ok` `lad_ladder_ok`（bool）、`paratope_geometry_flag`
  （bool；任一判据 fail 即 True）、`fr_aniso` `theta_deg` `cos_aim`
  `h2_epitope_contacts` `delta_h1_h3` `fr_fr_hbond_count`（诊断 metrics）、
  `geometry_status`（`ok` / `skipped:<reason>`）
- `geometry_status != ok` 的候选 `paratope_geometry_flag` 留空（不因解析失败惩罚——
  soft-flag-first）
- Output: `{RESULTS_DIR}/val/l3b/paratope_geometry.csv`（透传所有输入列 + 上述新列）
- Expected attrition: 0（soft flag，不删候选；仅影响 Pareto 排序）
...
- 工具输出 `dsasa_ratio` 列（= CDR3 dSASA ratio）；连续值直接进 Step 4.4 Pareto 作目标维度，`--threshold` 仅打印报告参考、**不物理删除**（遵循 changelog (24) 连续值进 Pareto 不 hard drop 原则）
- 注意 FR3 motif：longcdr3 scaffold 是 `DRFTISRDNAK`（非标准 `GRFTISRDNAK`），`cdr_boundaries.py` regex 已放宽为 `[A-Z]RF[TN]ISRDNAK`；个别临床 scaffold（如 8coh）FR3 仍可能不匹配 → `dsasa_status` 列记 `CDR boundary detection failed`，该候选 dSASA 列留空（不惩罚）
- Output: `{RESULTS_DIR}/val/l3b/dsasa_filter_results.csv`（透传所有输入列 + 追加 dSASA 列）
- 详见 `feedback_dsasa_filter.md`

**CDR 主导界面判断（`cdr_dominance_score`，与 dSASA 同步计算）：**

```bash
# Step 3.5b 单次调用：--all-cdrs 同时算 CDR dSASA ratio + cdr_dominance_score
# CSV 输入按 --cif-path-col（默认 representative_cif_path_primary）取每行 CIF，透传所有输入列
python ~/protein-design-utils/vhh/filter_dsasa.py \
  --input {RESULTS_DIR}/val/l3b/paratope_geometry.csv \
  --all-cdrs \
...
- `cdr_dominance_score` = Σ(CDR1 + CDR2 + CDR3 dSASA_abs) / total_interface_dSASA
...
- **Hard drop：** `cdr_dominance_score` < `L3_CDR_DOMINANCE_HARD_DROP`（默认 0.25）→ FR 堆积极度主导，paratope 贡献可忽略 → 写入 `cdr_dominance_hard_drop=True`；**工具不物理删除，下游按此列过滤**
- 写入 `dsasa_filter_results.csv`，追加列：`dsasa_ratio`、`cdr1/2/3_dsasa_ratio`、`cdr1/2/3_dsasa_abs`、`total_interface_dsasa`、`cdr_dominance_score`、`cdr_dominance_low_flag`、`cdr_dominance_hard_drop`、`dsasa_status`
...
- 用 BioPython NeighborSearch 计算 VHH–antigen 所有 Cβ–Cβ（Gly 用 Cα 代替）距离 < `--clash-dist`（默认 3.4Å）的残基对数
- **Hard drop：** `clash_pair_count` > `--clash-max`（`L3_CLASH_MAX_PAIRS`，默认 3）→ 写入 `clash_hard_drop=True`；**工具不物理删除，下游按此列过滤**
- L3.A 中已做粗 clash 预判；本步骤在 L3.B 复合物结构上精确计算，两者独立互补
- 结果写入 `consensus_ranked_dsasa.csv`（透传输入列 + 追加 `clash_pair_count`、`clash_hard_drop`、`clash_status`）
...
- **ipSAE Spearman 秩相关**（Step 3.5 各模型间）< 0.4 → 界面信号不可靠；回查 L3 threshold 是否过松，并考虑启用 3-model 模式或 Step 3.5 rescue 自适应 seed rescue（§3.5.3，已落地）。
...
- Step 2.mhcfilter 后：~510–810（全 soft flag，无 attrition；FR/CDR MHC 命中 → mhc_risk_score 进 Pareto，不删候选）
...
# 5 flag 乘性降权（cdr_dominance_low ×0.7 / anticonf_low ×0.85 / pose_diverged ×0.5 / ipsae_min_min_below_floor ×0.7 / paratope_geometry_flag ×0.70 Step 3.5a-bis C0/C2/C5/LAD 任一 fail（paratope 几何姿态可疑））
mult = 1.0
...
**8 个 Pareto 目标（含 generation_freq）（2026-05-12 新增 `seq_risk_count` 维度，配合 L2/L4 分工再校准）：**

| 来源列 | 方向 | 来源步骤 |
|--------|------|---------|
| `binding_composite` | max | 5 分量加权（ipSAE_min 0.40 + cluster_size 0.20 + AntiConf 0.15 + dSASA 0.15 + cdr_dominance 0.10）+ 5 flag 乘性降权（含 `paratope_geometry_flag` ×0.70 Step 3.5a-bis C0/C2/C5/LAD 任一 fail）；详见 scripts/config.py |
...
| `seq_risk_count` | min | Step 2.seqfilter 可救 soft flag 累加：`pi_extreme_flag + net_charge_extreme_flag + cdr_cys_unpaired_flag`（末项仅 `pathC_iggm`，2026-05-16 §2）（2026-05-12 新增维度；CDR glycan 单算 `glycan_count`） |
| `humanness_score` | max | L4.1（来自 Step 2.seqfilter ANARCI `v_identity`） |
| `generation_freq` | max | Step 2.dedup `gen_cluster_size × (boost if cross_path_count≥2 else 1.0)`；生成频率内生置信信号（§3.1）。已 wiring（2026-05-16 reconcile）；boost 因子 = `config.GENERATION_FREQ_CROSSPATH_BOOST`（1.5，surrogate 待真 L3.B 数据校准） |
...
| binding_composite | 用成熟序列新分数按 Step 4.4 binding_composite 5 分量公式重算（ipSAE_min 0.40 + cluster_size 0.20 + AntiConf 0.15 + dSASA 0.15 + cdr_dominance 0.10 + flag 乘性降权；权重见 scripts/config.py:BINDING_COMPOSITE_WEIGHTS） |
...
| 路径 | 触发条件 | 下游输入来源 |
|------|---------|------------|
| **跳过 Layer 5**（推荐首轮） | 无实验 hit 数据；默认路径 | Layer 4.5 `reviewed_candidates.csv` 直接进 Layer 6 |
...
> `{STRUCT_DIR}` = `{RESULTS_DIR}/mature/`（运行 Layer 5 时）或 `{RESULTS_DIR}/val/l3b/`（跳过 Layer 5 时——`mature/` 不存在，改用 L3.B 共折叠结构目录）。

```bash
modal run modal_pdb2png.py \
  --input-dir {STRUCT_DIR}/ \
...
3. **Immunogenicity scan is mandatory** (`reference_vhh_max_success_skill.md`): Step 2.mhcfilter (MHC I+II) cannot be skipped for therapeutic VHH.
...
- Never relax immunogenicity (Step 2.mhcfilter) or glycosylation (Step 2.seqfilter)
...
| unpaired cysteine（仅 Path A/B/D） | Step 2.seqfilter | 奇数 Cys 错配，结构不稳定（`pathC_iggm` 例外见下方 Soft Flag 表） |
...
| **非配对 Cys（仅 `pathC_iggm`）**（2026-05-16 §2 从 hard drop 改） | `cdr_cys_unpaired_flag` + `rescue_suggestion` | C→S/A 单点突变（IgGM 训练无 Cys 约束→杂散 CDR Cys） |
