...
**8 个 Pareto 目标（含 generation_freq，wiring pending）（2026-05-12 新增 `seq_risk_count` 维度，配合 L2/L4 分工再校准）：**
...
| `generation_freq` | max | Step 2.dedup `gen_cluster_size × (1.5 if cross_path_count≥2 else 1.0)`；生成频率内生置信信号（§3.1）。**⚠️ 代码 wiring pending** — `pareto_L4_ranker.py` 待整体 reconcile（脚本现停留 Round 2 旧版，与本表全面不符），reconcile 时把此目标接入并将 boost 1.5 落为 `config.GENERATION_FREQ_CROSSPATH_BOOST` |
